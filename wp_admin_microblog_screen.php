<?php
/**
 * Add help section to wp admin microblog main screen
 * @global type $wpam_admin_page
 * @return type 
 */
function wp_admin_blog_add_help_tab () {
 ...
 * @return string 
 */
function wp_admin_blog_print_message ($post, $user_info, $options, $level = 1) {
     $edit_button = '';
     $str = "'";
     $time = wp_admin_blog_datumsplit($post->date);
     $message_text = wp_admin_blog_replace_bbcode( wp_admin_blog_replace_url($post->text) );
...
function wp_admin_blog_page() {
   ...
   ?>
    ...
    <h2 style="padding-bottom: 10px;"><?php echo $wpam_blog_name;?></h2>
    ...
    <thead>
        ...
        <tr>
            ...
            <p><input name="tags" type="text" style="width:100%;" value="<?php _e('Tags (separate by comma)', 'wp_admin_blog');?>" onblur="if(this.value=='') this.value='<?php _e('Tags (separate by comma)', 'wp_admin_blog'); ?>';" onfocus="if(this.value=='<?php _e('Tags (separate by comma)', 'wp_admin_blog'); ?>') this.value='';" /></p>
            </div>
        ...
        <tr>
         ...
         if ( $search != '' || $author != '' || $tag != '' ) {
            ...
            if ($tag != '') {
               if ($where != "") {
                  ...
                  $where = "WHERE p.post_parent = '0' AND t.tag_ID = '$tag'";
               }
            }	
         ...
         if ($test == 0) {
            ...
            foreach ($post as $post) {
               ...
               echo wp_admin_blog_print_message($post, $user_info, $options);
               // print replies
               if ($search == '') {
                    $r = '';
                    $str = "'";
                    ...
                    foreach ( $replies as $reply ) {
                         if ( $reply->post_parent == $post->post_ID ) {
                              ...
                              $r = $r . '<td>' . wp_admin_blog_print_message($reply, $user_info, $options, 2) . '</td>';
                              $r = $r . '</tr>';
                         }
                    }
